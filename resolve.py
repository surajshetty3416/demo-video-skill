...
def resolve_meta(m):
    ...
    for i in range(n):
        if i < tin or i > min(tout, n - 1):
            reps[i] = 0
    # interior section deletions ("cut" is the legacy spelling of the key)
    for r in (m.get("cuts") or m.get("cut")) or []:
...
def needs_resolve(m):
    return bool(m.get("speed") or m.get("trim") or m.get("cuts") or m.get("cut")
                or m.get("cursorHide") or m.get("captions")
